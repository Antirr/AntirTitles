...
public class TitleGrantedHandler extends EntityEventSystem<EntityStore, TitleGrantEvent>
{
    public TitleGrantedHandler()
    {
    ...
    {
        return super.shouldProcessEvent(event) && event.type == TitleGrantEventType.GRANT;
    }
                       ...
                       @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull TitleGrantEvent event)
    {
        ...
        if (comp.hasTitle(title.getId()))
        {
            if (playerRef != null)
            {
                TitleUI.showError(playerRef, Message.translation("server.ui.titles.alreadyHave"));
            }
            return;
        }

        comp.addTitle(title.getId());

        TitleEffectContext ctx = new TitleEffectContext(ref, store, commandBuffer, title);
        for (ITitleEffect effect : title.getPassiveEffects()) effect.apply(ctx);

        if (playerRef != null)
        {
            TitleUI.showGranted(playerRef, title);
        }
    }
}
